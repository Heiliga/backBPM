...
import static ru.MTUCI.domain.security.RoleName.ROLE_ADMIN;
...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    @Transactional(readOnly = true)
    public Map<String, Set<RoleName>> getEmployeesAndTheirRoles() {
        Map<String, Set<RoleName>> EmployeesAndTheirRoles = new HashMap<>();
        employeeRepository.findAll().forEach(employee -> {
                EmployeesAndTheirRoles.put(employee.getFullName(), getSetRoleName(employee.getRoles()));
            }
        );
        return EmployeesAndTheirRoles;
    }

    @Transactional(readOnly = true)
    public Map<RoleName, Long> getMapCountOfEmployeesByRole() {
        Map<RoleName, Long> mapCountEmployeesByRole = new HashMap<>();
        roleRepository.findAll().forEach(role -> {
            Long countEmployee = employeeRepository.findAll().stream()
                .filter(user ->
                    user.getRoles().stream()
                        .anyMatch(role1 -> role1.getName() == role.getName()))
                .count();
            mapCountEmployeesByRole.put(role.getName(), countEmployee);
        });
        return mapCountEmployeesByRole;
    }
    ...
    private Set<RoleName> getSetRoleName(Set<Role> roles) {
